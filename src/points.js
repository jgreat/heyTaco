 ...
 *
 * @return {Promise<array>} An array of entries, each an object containing 'item' (string) and 'score'
 *                (integer) properties.
 */
const retrieveTopScores = async() => {
  ...
  const dbClient = await postgres.connect().catch( ( err ) => {
 ...
 *
 * @return {Promise<number>} The item's new score after the update has been applied.
 */
const updateScore = async( item ) => {
  ...
  await dbClient.query( '\
    INSERT INTO ' + scoresTableName + ' VALUES (\'' + item + '\', 1) \
    ON CONFLICT (item) DO UPDATE SET score = ' + scoresTableName + '.score +1; \
  ' );
