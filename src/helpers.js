 ...
 * @returns {array<String>} A list of 'user's being referred to (eg. 'U12345678')
 */
const extractTacoData = ( text ) => {
  const data = [...text.matchAll( /<@(U[A-Za-z0-9]{7,13})>.*?:taco:/g )];

  return data.map((match) => match[1])
}; // extractTacoData.
 ...
 */
const getTimestamp = () => {
  return Math.floor( Date.now() / MILLISECONDS_TO_SECONDS ).toString();
};

/**
 * Determines whether a number should be referred to as a plural - e.g. anything but 1 or -1.
 *
 * @param {number} number The number in question.
 ...
 * @param {string} ts    The timestamp the token was supplied with.
...
/**
 * Determines whether a string represents a Slack user ID - e.g. U12345678.
 *
