...
const getLeaderboardUrl = ( request ) => {
  ...
  return 'http://' + hostname + '/leaderboard?' + querystring.stringify( params );
 ...
 *
 * @returns {Promise<array>} An array, in rank order, of either human-readable Slack strings (if
...
const getForSlack = async( event) => {
  ...
  return slack.sendMessage( message, event );
 ...
 *
 * @param {object} request The Express request object that resulted in this handler being run.
 * @returns {Promise<string>} HTML for the browser.
 */
