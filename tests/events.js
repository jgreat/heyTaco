...
describe( 'handleSelfTaco', () => {

  const user = 'U12345678',
        channel = 'C12345678';

  it( 'logs an attempt by a user to increment their own score', () => {
    events.handleSelfTaco( user, channel );
    expect( console.log ).toHaveBeenCalledTimes( 1 );
  });

  it( 'gets a message from the \'self taco\' collection', () => {
    events.handleSelfTaco( user, channel );

    expect( messages.getRandomMessage )
      .toHaveBeenCalledTimes( 1 )
      .toHaveBeenCalledWith( 'selfPlus', 0, user );
  });
...
describe( 'handleTaco', () => {
  ...
  /** @returns {number} Returns a fake score. */
  ...
  it( 'calls the score updater to update an item\'s score', () => {
    ...
    events.handleTaco( item, channel );
  ...
  it.each([ [ 'plus', 1 ] ])(
    ...
    ( operationName, amount ) => {
      ...
      return events.handleTaco( item, channel ).then( () => {
        expect( messages.getRandomMessage )
          .toHaveBeenCalledTimes( 1 )
          .toHaveBeenCalledWith( operationName, amount, item, score );
      });
    }
  );
...
describe( 'handlers.message', () => {
  ...
  it( 'returns false if a user trying to Taco themselves', () => {
    const event = {
      type: eventType,
      text: '<@U12345678>Taco',
      user: 'U12345678'
